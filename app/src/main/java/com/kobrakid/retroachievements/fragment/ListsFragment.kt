...
class ListsFragment : Fragment(), RAAPICallback {
    ...
    private var consoleListLayoutManager: LinearLayoutManager? = null
    private var consoleAdapter: ConsoleAdapter? = null
    private var gameSummaryAdapter: GameSummaryAdapter? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        if (savedInstanceState == null) {
            ...
            consoleAdapter = ConsoleAdapter(this)
            gameSummaryAdapter = GameSummaryAdapter(context!!)
        }

        // Initialize views
        consoleListRecyclerView = view.findViewById(R.id.list_console)
        consoleListRecyclerView!!.adapter = consoleAdapter
        consoleListLayoutManager = LinearLayoutManager(context)
    ...
    override fun callback(responseCode: Int, response: String) {
        ...
        if (responseCode == RAAPIConnection.RESPONSE_GET_CONSOLE_IDS) {
                ...
                if (hideEmptyConsoles) {
                    for (i in 0 until reader.length()) {
                        ...
                        instance?.diskIO()?.execute {
                            ...
                            if (current.size > 0 && current[0].gameCount == 0) {
            ...
            try {
                ...
                if (view != null) populateGamesView(view, true)
            } catch (e: JSONException) {
                e.printStackTrace()
            }
        }
